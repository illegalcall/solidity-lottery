...
describe('Lottery', () => {
	...
	describe('Lucky Numbers Generation', () => {
		...
		it('Should confirm lucky number import', async () => {
			result = await contract.getLotteryLuckyNumbers(
				lotteryId
			);
			expect(result).to.have.lengthOf(0);
	...
	describe('Buying of Tickets', () => {
		...
		it('Should confirm ticket purchase', async () => {
			...
			result = await contract.getLotteryParticipants(
				lotteryId
			);
			expect(result).to.have.lengthOf(0);
	...
	describe('Selecting Winners', () => {
		...
		it('Should confirm random selection of winners', async () => {
			...
			await contract.randomlySelectWinners(
				lotteryId,
				numberOfWinners
			);

			result = await contract.getLotteryResult(lotteryId);
			expect(result.winners).to.have.lengthOf(
				numberOfWinners
			);
		});
	});
	...
	describe('Paying Winners', () => {
		...
		it('Should confirm random selection of winners', async () => {
			...
			result = await contract.getLottery(lotteryId);
			expect(result.winners.toNumber()).to.be.equal(
				numberOfWinners
			);
