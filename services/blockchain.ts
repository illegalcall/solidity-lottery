...
const fromWei = (num: number) =>
	ethers.utils.formatEther(num);
...
const monitorWalletConnection = async () => {
	try {
		...
		if (accounts.length) {
			...
			reportError(
				'Please, connect wallet, no accounts found.'
			);
		}
	} catch (error) {
		reportError(error);
...
const getLottery = async (id: string) => {
	const contract = await ssrEthereumContract();
	const lottery = await contract.getLottery(id);
	return structureLotteries([lottery])[0];
};

const getLuckyNumbers = async (id: string) => {
	const contract = await ssrEthereumContract();
	const luckyNumbers =
		await contract.getLotteryLuckyNumbers(id);
	return luckyNumbers;
};

const getPurchasedNumbers = async (id: string) => {
	const contract = await ssrEthereumContract();
	const participants =
		await contract.getLotteryParticipants(id);
...
const getLotteryResult = async (id: string) => {
	const contract = await ssrEthereumContract();
...
const exportLuckyNumbers = async (
	...
	try {
		...
		tx = await contract.importLuckyNumbers(
			id,
			luckyNumbers,
			{
				from: wallet,
...
const performDraw = async (
	...
	try {
		...
		tx = await contract.randomlySelectWinners(
			id,
			numberOfWinners,
			{
				from: wallet,
...
const truncate: (
	...
	endChars: number,
	maxLength: number
) => string = (text, startChars, endChars, maxLength) => {
	if (text.length > maxLength) {
		let start = text.substring(0, startChars);
		let end = text.substring(
